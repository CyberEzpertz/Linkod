...
export default function BookAppointmentPage() {
  ...
  const [unavailableDates] = useState<Date[]>(() => {
    ...
    for (let monthOffset = 0; monthOffset < 6; monthOffset++) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (dateNum % 3 === 0) { // Make every third date unavailable
  ...
  const timeSlots = date ? DEFAULT_TIME_SLOTS.map(time => {
    ...
    const dateNum = date.getDate() + date.getMonth() + date.getFullYear() + timeNum;
  ...
  const isBookedDateSelected = date && 
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="flex flex-col items-center p-6 bg-white rounded-lg shadow-lg">
          <Calendar
            ...
            classNames={{
              ...
              day_disabled: "text-red-500 line-through cursor-not-allowed hover:bg-transparent",
            }}
          />
        </div>
        ...
        <div className="flex flex-col space-y-6 w-full">
          <div className="space-y-2">
            <h2 className="text-lg font-semibold text-black">Purpose of the Appointment</h2>
          ...
          {availableSlots.length > 0 && (
            <>
              <h2 className="text-lg font-semibold text-black">Available Time Slots</h2>
              <div className="flex flex-col space-y-2">
                {availableSlots.map((slot) => (
                  <Button
                    key={slot.time}
                    variant="outline"
                    className={`flex justify-between items-center py-6 px-6 text-lg h-16 ${
                      selectedTime === slot.time ? "border-black bg-gray-50" : ""
                    }`}
          ...
          {unavailableSlots.length > 0 && (
            <>
              <h2 className="text-lg font-semibold text-black mt-6">Unavailable Time Slots</h2>
              <div className="flex flex-col space-y-2">
                {unavailableSlots.map((slot) => (
                  <Button
                    key={slot.time}
                    variant="outline"
                    className="flex justify-between items-center py-6 px-6 text-lg h-16 border-red-500 text-gray-500 cursor-not-allowed"
                    disabled
                  >
                    <span className="text-lg">{slot.time}</span>
        ...
        <div className="space-y-4">
          <Button
            className="w-full py-8 text-xl font-semibold bg-black hover:bg-gray-800 text-white"
          ...
          <Button
            variant="outline"
            className="w-full py-8 text-xl font-semibold border-2 border-black hover:bg-gray-50"
